...
class SentimentAnalysis:
	...
	def writeoutput(self):
		self.outputdf.to_csv("output.csv", index=False)
	...
	def aggregateinformation(self, df):
	    ...
	    self.outputdf=self.outputdf.append({'Stockname':self.stockname,'Mentions':self.mentions,'countpositive':countpositive,'countnegative':countnegative,'countneutral':countneutral, 'verbs':verbsoccurences.verbs.tolist()}, ignore_index=True)

	def run(self):
#		for jsonline in range(len(self.jsondata['stock_list'])):
		for jsonline in range(2):
